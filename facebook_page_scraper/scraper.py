...
class Facebook_scraper:
    ...
    def __handle_popup(self, layout):
        ...
        try:
            if layout == "old":
                ...
                Utilities._Utilities__close_cookie_consent_modern_layout(self.__driver)
    ...
    def scrap_to_json(self):
        ...
        check = 0
        print("[+]  Grabbing Posts of last {} days".format(str(self.days_count)))
        while (
            (
                datetime.strptime(
                    list(self.__data_dict.values())[-1].get("posted_on").split("T")[0],
                    "%Y-%m-%d",
                ).date()
                >= (datetime.now() - timedelta(days=self.days_count)).date()
            )
            if self.__data_dict
            else True
        ):
            if self.__data_dict:
                print(
                    ...
                    str(
                        datetime.strptime(
                            list(self.__data_dict.values())[-1]
                            .get("posted_on")
                            .split("T")[0],
                            "%Y-%m-%d",
                        ).date()
            ...
            if check == 3 and len(self.__data_dict) == 1:
                break
            else:
                check = check + 1
        ...
        with open(
            ...
            for key in json_data:
                ...
                row = {
                    ...
                    "shares": json_data[key]["shares"],
                    "likes": json_data[key]["reactions"]["likes"],
                    "loves": json_data[key]["reactions"]["loves"],
                    "wow": json_data[key]["reactions"]["wow"],
                    "cares": json_data[key]["reactions"]["cares"],
                    "sad": json_data[key]["reactions"]["sad"],
                    "angry": json_data[key]["reactions"]["angry"],
                    "haha": json_data[key]["reactions"]["haha"],
                    "reactions_count": json_data[key]["reaction_count"],
                    "comments": json_data[key]["comments"],
                    "content": json_data[key]["content"],
                    "posted_on": json_data[key]["posted_on"],
                    "video": json_data[key]["video"],
                    "image": " ".join(json_data[key]["image"]),
    ...
    def scrap_to_csv(self, filename, directory=os.getcwd()):
        try:
            data = (
                self.scrap_to_json()
            )  # get the data in JSON format from the same class method
    ...
    def __remove_duplicates(self, all_posts):
        ...
        if (
            len(self.__extracted_post) == 0
        ):  # if self.__extracted_post is empty that means it is first extraction
    ...
    def __find_elements(self, name):
        ...
        for post in all_posts:
            try:
                ...
                all_hrefs_in_react = (
                    Finder._Finder__find_reaction(
                        self.__layout,
                        reactions_all,
                    )
                    if type(reactions_all) != str
                    else ""
                )
                # if hrefs were found
                ...
                total_reaction_count = (
                    Scraping_utilities._Scraping_utilities__count_reaction(reactions)
                )
