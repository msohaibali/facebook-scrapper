...
class Facebook_scraper:
    ...
    def __init__(self, page_name, posts_count=10, days_count=3, browser="chrome", proxy=None, timeout=600, headless=True):
    ...
    def scrap_to_json(self):
        ...
        while (datetime.strptime(list(self.__data_dict.values())[-1].get("posted_on").split("T")[0], "%Y-%m-%d").date() >= (datetime.now() - timedelta(days=self.days_count)).date()) if self.__data_dict else True:
            if self.__data_dict:
                print("[+]  Last Post Date: ", str(datetime.strptime(list(self.__data_dict.values())[-1].get("posted_on").split("T")[0], "%Y-%m-%d").date()))
